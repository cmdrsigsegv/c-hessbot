...
const char *BOT_NAME = "default_bot";
Move best_root;
...
static int evaluate(void) {
	...
	for (sq = 0; sq < 128; sq++) {
		...
		t = PTYPE(p);
		c = PCOL(p);
		idx = (c == WHITE) ? (7 - RANKOF(sq)) * 8 + FILEOF(sq)
...
static double elapsed(void) {
	...
	return (now.tv_sec - t_start.tv_sec) +
	       1e-9 * (now.tv_nsec - t_start.tv_nsec);
}
